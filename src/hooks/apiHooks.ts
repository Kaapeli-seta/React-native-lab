...
const useUser = () => {
  ...
  const postRegister = async (credentials: RegisterCredentials) => {
    ...
    try {
      return await fetchData<UserResponse>(process.env.EXPO_PUBLIC_AUTH_API + '/users', options);
    } catch (error) {
  ...
  const getUserById = async (id: number) => {
    return await fetchData<UserWithNoPassword>(process.env.EXPO_PUBLIC_AUTH_API + '/users/' + id);
  };
...
const useLike = () => {
  const postLike = async (media_id: number, token: string) => {
    ...
    return await fetchData<MessageResponse>(process.env.EXPO_PUBLIC_MEDIA_API + '/likes', options);
  };
